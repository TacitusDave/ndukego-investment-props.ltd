...
import { toggleEstateFeatured } from "@/lib/actions";

export function EstateFeaturedToggle({
  estateId,
  featured,
}: {
  estateId: string;
  ...
  function handleToggle() {
    ...
    startTransition(async () => {
      const result = await toggleEstateFeatured(estateId, !featured);
      if (result.error) {
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Featured estates appear in the homepage spotlight section.
          </p>
        </div>
        <div

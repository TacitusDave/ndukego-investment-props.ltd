...
import { hardDeleteEstate } from "@/lib/actions";
...
export function EstateDeleteButton({ estateId, estateName }: Props) {
  ...
  const confirmed = typed.trim() === estateName.trim();
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      const result = await hardDeleteEstate(estateId);
      if (result.error) {
          ...
          <div className="relative w-full max-w-md rounded-xl bg-white shadow-2xl border border-gray-200 p-6 space-y-5 mx-4 overflow-hidden">
            ...
            {isPending && (
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 rounded-xl bg-white/95">
                ...
                <p className="text-sm font-medium text-gray-700">Deleting estate…</p>
                <p className="text-xs text-gray-400 text-center px-6">
                  This may take a few seconds while data is being removed. Please wait.
                </p>
              </div>
            )}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="text-base font-semibold text-gray-900">Delete estate permanently?</h2>
                <p className="text-sm text-gray-500 mt-1">
                  This will permanently remove{" "}
                  <span className="font-medium text-gray-700">&ldquo;{estateName}&rdquo;</span>{" "}
                  from the database and all associated data. All properties must be deleted first.
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="Type the estate name exactly"

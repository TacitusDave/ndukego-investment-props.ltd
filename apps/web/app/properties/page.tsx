...
const PropertiesMapClient = dynamic(
  () =>
    import("@/components/properties-map-client").then((m) => m.PropertiesMapClient),
  { ssr: false, loading: () => <div className="w-full h-full flex items-center justify-center text-sm text-gray-400">Loading map…</div> }
);
    ...
    <>
      ...
      {isMapView && (
        <section className="relative" style={{ height: "calc(100vh - 200px)", minHeight: "500px" }}>
          ...
          {mapPins.length > 0 && (
            <PropertiesMapClient properties={mapPins} />
          )}
        </section>
      )}
              ...
              <>
                ...
                {meta && meta.totalPages > 1 && (
                  <div className="mt-10 flex items-center justify-center gap-2">
                    {meta.page > 1 && (
                      <Link
                        href={`/properties?page=${meta.page - 1}${search ? `&search=${search}` : ""}${category ? `&category=${category}` : ""}${state ? `&state=${state}` : ""}`}
